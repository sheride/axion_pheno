                      ...
                      const std::map<std::string,std::string>& parameters)
{
  ...
  // ===== Selections ===== //
  Manager()->AddCut("1_30.0 > PT > 30.0");
  Manager()->AddCut("2_sdETA ( jets[1] jets[2] ) > 3.6 or sdETA ( jets[1] jets[2] ) < -3.6");
  Manager()->AddCut("3_M ( jets[1] jets[2] ) > 750.0");

  // ===== Histograms ===== //
  Manager()->AddHisto("1_PT", 100,0.0,1000.0);
  Manager()->AddHisto("2_ETA", 200,0.0,2000.0);
  Manager()->AddHisto("3_PHI", 160,-8.0,8.0);
  Manager()->AddHisto("4_PT", 64,-3.2,3.2);
  Manager()->AddHisto("5_ETA", 100,0.0,1000.0);
  Manager()->AddHisto("6_PHI", 160,-8.0,8.0);
  Manager()->AddHisto("7_DELTAR", 64,-3.2,3.2);
  Manager()->AddHisto("8_M", 75,0.0,15.0);
  Manager()->AddHisto("9_sdETA", 160,-8.0,2000.0);
  Manager()->AddHisto("10_M", 100,2.4,8.0);
  Manager()->AddHisto("11_PT", 400,0.0,1000.0);
  Manager()->AddHisto("12_PT", 80,0.0,1000.0);
  Manager()->AddHisto("13_THT", 400,0.0,2000.0);
  Manager()->AddHisto("14_MET", 80,0.0,4000.0);
  Manager()->AddHisto("15_TET", 200,0.0,1000.0);
  Manager()->AddHisto("16_DELTAR", 80,0.0,8000.0);
  Manager()->AddHisto("17_sdETA", 100,-8.0,8.0);
...
bool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  //   * Cut: select 30.0 > PT > 30.0, regions = []
  {
    ...
    {
    {
      ...
      {
        if (_P_jets_I1I_PTorderingfinalstate_REG_[ind[0]]->pt()>30.0) {filter[0]=true; break;}
      }
    }
    }
    {
    {
      ...
      {
        if (_P_jets_I2I_PTorderingfinalstate_REG_[ind[0]]->pt()>30.0) {filter[1]=true; break;}
      }
    }
    }
    MAbool filter_global = (filter[0] && filter[1]);
    if(!Manager()->ApplyCut(filter_global, "1_30.0 > PT > 30.0")) return true;
  }

  // Event selection number 2
  //   * Cut: select sdETA ( jets[1] jets[2] ) > 3.6 or sdETA ( jets[1] jets[2] ) < -3.6, regions = []
  {
    std::vector<MAbool> filter(2,false);
    {
    {
    ...
    MAbool filter_global = (filter[0] || filter[1]);
    if(!Manager()->ApplyCut(filter_global, "2_sdETA ( jets[1] jets[2] ) > 3.6 or sdETA ( jets[1] jets[2] ) < -3.6")) return true;
  }

  // Event selection number 3
  //   * Cut: select M ( jets[1] jets[2] ) > 750.0, regions = []
  {
    ...
    if (matched) continue;
        ...
        if (q.m()>750.0) {filter[0]=true; break;}
      }
      }
    }
    }
    MAbool filter_global = (filter[0]);
    if(!Manager()->ApplyCut(filter_global, "3_M ( jets[1] jets[2] ) > 750.0")) return true;
  }
  ...
  {
  {
    ...
    value-=_P_jets_I2I_PTorderingfinalstate_REG_[ind[1]]->eta();
      Manager()->FillHisto("9_sdETA", value);
    }
    }
  }
  }
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_a_I1I_PTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("11_PT", _P_a_I1I_PTorderingfinalstate_REG_[ind[0]]->pt());
    }
  }
  }
  ...
  {
  {
    ...
    {
      Manager()->FillHisto("12_PT", _P_a_I2I_PTorderingfinalstate_REG_[ind[0]]->pt());
    }
  }
  }
  ...
  //   * Plot: THT
  {
    Manager()->FillHisto("13_THT", PHYSICS->Transverse->EventTHT(event.mc()));
  }

  // Histogram number 14
  //   * Plot: MET
  {
    Manager()->FillHisto("14_MET", PHYSICS->Transverse->EventMET(event.mc()));
  }

  // Histogram number 15
  //   * Plot: TET
  {
    Manager()->FillHisto("15_TET", PHYSICS->Transverse->EventTET(event.mc()));
  }
  ...
  {
  {
    ...
    {
    MAuint32 b[1];
    for (b[0]=0;b[0]<_P_a_I2I_PTorderingfinalstate_REG_.size();b[0]++)
    {
     if ( _P_a_I1I_PTorderingfinalstate_REG_[a[0]] == _P_a_I2I_PTorderingfinalstate_REG_[b[0]] ) continue;
      Manager()->FillHisto("16_DELTAR", _P_a_I1I_PTorderingfinalstate_REG_[a[0]]->dr(_P_a_I2I_PTorderingfinalstate_REG_[b[0]]));
    }
    }
  }
  }
  ...
  {
  {
    ...
    MAdouble64 value=0;
    value+=_P_a_I1I_PTorderingfinalstate_REG_[ind[0]]->eta();
    value-=_P_a_I2I_PTorderingfinalstate_REG_[ind[1]]->eta();
      Manager()->FillHisto("17_sdETA", value);
    }
    }
  }
  }
