                      ...
                      const std::map<std::string,std::string>& parameters)
{
  ...
  // ===== Selections ===== //
  Manager()->AddCut("1_M ( a[1] a[2] ) > 500.0");
  Manager()->AddCut("2_PT ( a[1] ) > 300.0");
  Manager()->AddCut("3_M ( jets[1] jets[2] ) > 750.0");
  Manager()->AddCut("4_sdETA ( jets[1] jets[2] ) > 3.6 or sdETA ( jets[1] jets[2] ) < -3.6");

  // ===== Histograms ===== //
  Manager()->AddHisto("1_PT", 200,0.0,2000.0);
  Manager()->AddHisto("2_ETA", 160,-8.0,8.0);
  Manager()->AddHisto("3_PHI", 64,-3.2,3.2);
  Manager()->AddHisto("4_PT", 100,0.0,1000.0);
  Manager()->AddHisto("5_ETA", 160,-8.0,8.0);
  Manager()->AddHisto("6_PHI", 64,-3.2,3.2);
  Manager()->AddHisto("7_DELTAR", 75,0.0,15.0);
  Manager()->AddHisto("8_M", 160,120.0,2000.0);
  Manager()->AddHisto("9_sdETA", 100,2.4,8.0);
  Manager()->AddHisto("10_M", 400,0.0,1000.0);
  Manager()->AddHisto("11_PT", 80,0.0,1000.0);
  Manager()->AddHisto("12_PT", 400,0.0,2000.0);
  Manager()->AddHisto("13_THT", 80,0.0,4000.0);
  Manager()->AddHisto("14_MET", 200,0.0,1000.0);
  Manager()->AddHisto("15_TET", 80,0.0,8000.0);
  Manager()->AddHisto("16_DELTAR", 100,0.0,6.28);
...
bool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  //   * Cut: select M ( a[1] a[2] ) > 500.0, regions = []
  {
    ...
    if (matched) continue;
        ...
        if (q.m()>500.0) {filter[0]=true; break;}
      }
      }
    }
    }
    MAbool filter_global = (filter[0]);
    if(!Manager()->ApplyCut(filter_global, "1_M ( a[1] a[2] ) > 500.0")) return true;
  }

  // Event selection number 2
  //   * Cut: select PT ( a[1] ) > 300.0, regions = []
  {
    std::vector<MAbool> filter(1,false);
    {
    {
    ...
    MAbool filter_global = (filter[0]);
    if(!Manager()->ApplyCut(filter_global, "2_PT ( a[1] ) > 300.0")) return true;
  }
  ...
  //   * Cut: select sdETA ( jets[1] jets[2] ) > 3.6 or sdETA ( jets[1] jets[2] ) < -3.6, regions = []
  {
    ...
    if(!Manager()->ApplyCut(filter_global, "4_sdETA ( jets[1] jets[2] ) > 3.6 or sdETA ( jets[1] jets[2] ) < -3.6")) return true;
  }
