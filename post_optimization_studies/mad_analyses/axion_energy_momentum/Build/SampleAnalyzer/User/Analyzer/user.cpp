                      ...
                      const std::map<std::string,std::string>& parameters)
{
  ...
  Manager()->AddHisto("2_P", 200,0.0,3000.0);
...
bool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  {
      _P_axPTorderingfinalstate_REG_.clear();
  }
  ...
  // Filling particle containers
  {
    ...
    {
      if (isP__axPTorderingfinalstate((&(event.mc()->particles()[i])))) _P_axPTorderingfinalstate_REG_.push_back(&(event.mc()->particles()[i]));
    }
  }
  ...
  {
    MAuint32 ind[1];
    for (ind[0]=0;ind[0]<_P_axPTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("1_E", _P_axPTorderingfinalstate_REG_[ind[0]]->e());
    }
  }
  }
  ...
  {
  {
    ...
    for (ind[0]=0;ind[0]<_P_axPTorderingfinalstate_REG_.size();ind[0]++)
    {
      Manager()->FillHisto("2_P", _P_axPTorderingfinalstate_REG_[ind[0]]->p());
    }
  }
  }
