                      ...
                      const std::map<std::string,std::string>& parameters)
{
  ...
  Manager()->AddCut("2_sdETA ( jets[1] jets[2] ) > 5.6 and M ( jets[1] jets[2] ) > 2000.0");
...
bool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  //   * Cut: select sdETA ( jets[1] jets[2] ) > 5.6 and M ( jets[1] jets[2] ) > 2000.0, regions = []
  {
    ...
    if (matched) continue;
        ...
        q+=_P_jets_I1I_PTorderingfinalstate_REG_[ind[0]]->momentum();
        q+=_P_jets_I2I_PTorderingfinalstate_REG_[ind[1]]->momentum();
        if (q.m()>2000.0) {filter[1]=true; break;}
      }
      }
    }
    }
    MAbool filter_global = (filter[0] && filter[1]);
    if(!Manager()->ApplyCut(filter_global, "2_sdETA ( jets[1] jets[2] ) > 5.6 and M ( jets[1] jets[2] ) > 2000.0")) return true;
  }
