                      ...
                      const std::map<std::string,std::string>& parameters)
{
  ...
  // ===== Selections ===== //
  Manager()->AddCut("1_PT ( a[1] ) > 350.0 and M ( a[1] a[2] ) > 400.0");
  Manager()->AddCut("2_( sdETA ( jets[1] jets[2] ) > 3.1 or sdETA ( jets[1] jets[2] ) < -3.1 ) and M ( jets[1] jets[2] ) > 1500.0");
...
bool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  //   * Cut: select PT ( a[1] ) > 350.0 and M ( a[1] a[2] ) > 400.0, regions = []
  {
    ...
    {
    {
      ...
      {
        if (_P_a_I1I_PTorderingfinalstate_REG_[ind[0]]->pt()>350.0) {filter[0]=true; break;}
      }
    }
    }
    {
    ...
    if (matched) continue;
        ...
        if (q.m()>400.0) {filter[1]=true; break;}
      }
      }
    }
    }
    MAbool filter_global = (filter[0] && filter[1]);
    if(!Manager()->ApplyCut(filter_global, "1_PT ( a[1] ) > 350.0 and M ( a[1] a[2] ) > 400.0")) return true;
  }

  // Event selection number 2
  //   * Cut: select ( sdETA ( jets[1] jets[2] ) > 3.1 or sdETA ( jets[1] jets[2] ) < -3.1 ) and M ( jets[1] jets[2] ) > 1500.0, regions = []
  {
    ...
    if(!Manager()->ApplyCut(filter_global, "2_( sdETA ( jets[1] jets[2] ) > 3.1 or sdETA ( jets[1] jets[2] ) < -3.1 ) and M ( jets[1] jets[2] ) > 1500.0")) return true;
  }
