                      ...
                      const std::map<std::string,std::string>& parameters)
{
  ...
  Manager()->AddCut("2_PT ( a[1] ) > 350.0 and M ( a[1] a[2] ) > 200.0");
...
bool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  //   * Cut: select PT ( a[1] ) > 350.0 and M ( a[1] a[2] ) > 200.0, regions = []
  {
    ...
    if (matched) continue;
        ...
        if (q.m()>200.0) {filter[1]=true; break;}
      }
      }
    }
    }
    MAbool filter_global = (filter[0] && filter[1]);
    if(!Manager()->ApplyCut(filter_global, "2_PT ( a[1] ) > 350.0 and M ( a[1] a[2] ) > 200.0")) return true;
  }
