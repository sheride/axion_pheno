                      ...
                      const std::map<std::string,std::string>& parameters)
{
  ...
  Manager()->AddCut("2_PT ( a[1] ) > 100.0 and M ( a[1] a[2] ) > 400.0");
...
bool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  //   * Cut: select PT ( a[1] ) > 100.0 and M ( a[1] a[2] ) > 400.0, regions = []
  {
    ...
    if(!Manager()->ApplyCut(filter_global, "2_PT ( a[1] ) > 100.0 and M ( a[1] a[2] ) > 400.0")) return true;
  }
