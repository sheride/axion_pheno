                      ...
                      const std::map<std::string,std::string>& parameters)
{
  ...
  Manager()->AddCut("2_PT ( a[1] ) > 200.0 and M ( a[1] a[2] ) > 550.0");
...
bool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  //   * Cut: select PT ( a[1] ) > 200.0 and M ( a[1] a[2] ) > 550.0, regions = []
  {
    ...
    {
    {
      ...
      {
        if (_P_a_I1I_PTorderingfinalstate_REG_[ind[0]]->pt()>200.0) {filter[0]=true; break;}
      }
    }
    }
    {
    ...
    if(!Manager()->ApplyCut(filter_global, "2_PT ( a[1] ) > 200.0 and M ( a[1] a[2] ) > 550.0")) return true;
  }
